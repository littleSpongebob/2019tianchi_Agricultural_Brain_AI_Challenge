...
_DATASET_NAME_1024 = DatasetDescriptor( 
    splits_to_sizes={ 
        'train': 4531, 
        'val': 500,         # 这里根据Segmentation中的.txt文件名对应，
        'trainval': 5031    # 数字代表对应数据集包含图像的数量
    }, 
    num_classes=5,   # 类别数目，包括背景
    ignore_label=255,   # 有些数据集标注有白色描边（VOC 2012），不代表任何实际类别
)

_DATASET_NAME_1024_A = DatasetDescriptor(
    splits_to_sizes={
        'train': 6652,
        'val': 500,         # 这里根据Segmentation中的.txt文件名对应，
        'trainval': 7152    # 数字代表对应数据集包含图像的数量
    },
    num_classes=5,   # 类别数目，包括背景
    ignore_label=255,   # 有些数据集标注有白色描边（VOC 2012），不代表任何实际类别
)

_DATASET_NAME_1024_A81 = DatasetDescriptor(
    splits_to_sizes={
        'train': 6675,
        'val': 500,         # 这里根据Segmentation中的.txt文件名对应，
        'trainval': 7175    # 数字代表对应数据集包含图像的数量
    },
    num_classes=5,   # 类别数目，包括背景
    ignore_label=255,   # 有些数据集标注有白色描边（VOC 2012），不代表任何实际类别
)

_DATASET_NAME_1024_preliminary = DatasetDescriptor( 
    splits_to_sizes={ 
        'train': 5103, 
        'val': 500,         # 这里根据Segmentation中的.txt文件名对应，
        'trainval': 5603    # 数字代表对应数据集包含图像的数量
    }, 
    num_classes=5,   # 类别数目，包括背景
    ignore_label=255,   # 有些数据集标注有白色描边（VOC 2012），不代表任何实际类别
)
...
_DATASETS_INFORMATION = {
    ...
    'remote_sensing_data_1024': _DATASET_NAME_1024,
    'remote_sensing_data_1024_add_preliminary': _DATASET_NAME_1024_preliminary,
    'remote_sensing_data_1024_a':_DATASET_NAME_1024_A,
    'remote_sensing_data_1024_a81':_DATASET_NAME_1024_A81

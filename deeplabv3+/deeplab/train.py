...
def _build_deeplab(iterator, outputs_to_num_classes, ignore_label):
  ...
  print('FLAGS.fine_tune_batch_norm={}'.format(FLAGS.fine_tune_batch_norm))
...
def main(unused_argv):
  ...
  with graph.as_default():
    ...
    with tf.device(tf.train.replica_device_setter(ps_tasks=FLAGS.num_ps_tasks)):
      ...
      #     total_num_parameters += np.array(v.get_shape().as_list()).prod()
      #     print(v.name)
      # print(total_num_parameters)
      # backbonebn_var_list = [t for t in tf.trainable_variables() if
      #                        FLAGS.model_variant in t.name and 'BatchNorm' in t.name]
      # freeze_backbonebn_var_list = [t for t in tf.trainable_variables() if t not in backbonebn_var_list]
      # for v in freeze_backbonebn_var_list:
      #     print(v.name)
      ...
      with tf.contrib.tfprof.ProfileContext(
        ...
        with tf.train.MonitoredTrainingSession(
            ...
            save_checkpoint_steps=FLAGS.save_interval_secs,

...
def multi_category_focal_loss1(y_true, y_pred):
  ...
  alpha = tf.constant([0.3, 0.9, 0.9, 1.1, 2.0], dtype=tf.float32)
...
def add_softmax_cross_entropy_loss_for_each_scale(scales_to_logits,
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    label1_weight = 3  # object1
    label2_weight = 2.5  # object2
    label3_weight = 2  # object3
    label4_weight = 4  # object4


    # 添加了a榜数据以后的权重
    # label0_weight = 0.9  # background
    # label1_weight = 3  # object1
    # label2_weight = 2.5  # object2
    # label3_weight = 2  # object3
    # label4_weight = 4  # object4
    ...
    not_ignore_mask = not_ignore_mask * tf.to_float(tf.not_equal(scaled_labels, ignore_label))
    ...
    if top_k_percent_pixels == 1.0:
      ...
      # lovasz_loss = L.lovasz_softmax(probas=softmax_logits,labels=scaled_labels,per_image=False)
      # tf.losses.add_loss(lovasz_loss)
      
      # focal_loss = multi_category_focal_loss1(y_true=one_hot_labels, y_pred=tf.reshape(softmax_logits, shape=[-1, num_classes]))
      # tf.losses.add_loss(3*focal_loss)
      ...
      with tf.name_scope(loss_scope, 'softmax_hard_example_mining',
        ...
        if hard_example_mining_step == 0:
          ...
          print('top_k_percent_pixels:{}'.format(top_k_percent_pixels))
...
def get_model_learning_rate(learning_policy,
  ...
  if learning_policy == 'step':
    ...
    learning_rate = tf.train.polynomial_decay(
        ...
        adjusted_global_step,
        training_number_of_steps,
        end_learning_rate=base_learning_rate/2,
        power=learning_power)
  else:
    raise ValueError('Unknown learning policy.')

  adjusted_slow_start_learning_rate = slow_start_learning_rate
  print('slow_start_burnin_type={}'.format(slow_start_burnin_type))
  if slow_start_burnin_type == 'linear':
    ...
    print('adjusted_slow_start_learning_rate={}'.format(adjusted_slow_start_learning_rate))
